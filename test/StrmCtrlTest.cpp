...
void test_TextSend_NoPeer_ReturnsFalse()
{
    StrmCtrlTestSuite fixture;
    ASSERT_TRUE(fixture.master_->start());

    EXPECT_FALSE(fixture.master_->sendPrefixedMessage("test:", "payload"));
    EXPECT_FALSE(fixture.slave_->sendPrefixedMessage("test:", "payload"));
}

void test_TextSend_ReturnsTrue_WithActivePeer()
{
    StrmCtrlTestSuite fixture;
    ASSERT_TRUE(fixture.master_->start());

    std::atomic<bool> received = false;
    ASSERT_TRUE(fixture.slave_->registerPrefixCallback(
        "test:",
    ...
    ASSERT_TRUE(waitUntil([&]()
                          { return fixture.slave_->isConnected(); }, 5s));

    EXPECT_TRUE(fixture.master_->sendPrefixedMessage("test:", "payload"));
    ASSERT_TRUE(waitUntil([&]()
                          { return received.load(); }, 5s));
}

void test_SingleSlave_RejectsSecondClient()
{
    StrmCtrlTestSuite fixture;
    Slave second_slave;
    second_slave.setConnectionCallback([](bool, const std::string &) {});
    ...
    std::atomic<bool> second_disconnected = false;
    second_slave.setConnectionCallback([&](bool connected, const std::string &)
                                       {
        if (!connected) {
            second_disconnected = true;
        } });

    ASSERT_TRUE(second_slave.connect("127.0.0.1", fixture.signaling_port_, fixture.rtp_port_));
    ASSERT_TRUE(waitUntil([&]()
                          { return second_disconnected.load(); }, 5s));
    EXPECT_FALSE(second_slave.isConnected());
    second_slave.disconnect();
}
...
void test_Robustness_StopWhilePushing()
{
    StrmCtrlTestSuite fixture;
    fixture.master_->setCodecConfig(CodecConfig::makeOpenH264(640, 480, 30, 500));
    fixture.master_->setAudioConfig(AudioConfig::makeAAC(48000, 2, 64000));
    ...
    std::thread video_thread([&]()
        ...
        while (keep_pushing.load()) {
            fixture.master_->pushVideoFrame(
                test::createDummyVideoFrame(640, 480, AV_PIX_FMT_YUV420P));
...
void run_all_tests()
{
    ...
    try
    {
        test_MasterStart_PortConflict();
        std::cout << "[       OK ] MasterStart_PortConflict\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] MasterStart_PortConflict: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_SlaveConnect_Success();
        std::cout << "[       OK ] SlaveConnect_Success\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] SlaveConnect_Success: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_Disconnection_Graceful();
        std::cout << "[       OK ] Disconnection_Graceful\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] Disconnection_Graceful: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_TextChat_MasterToSlave();
        std::cout << "[       OK ] TextChat_MasterToSlave\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] TextChat_MasterToSlave: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_TextChat_SlaveToMaster();
        std::cout << "[       OK ] TextChat_SlaveToMaster\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] TextChat_SlaveToMaster: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_TextSend_NoPeer_ReturnsFalse();
        std::cout << "[       OK ] TextSend_NoPeer_ReturnsFalse\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] TextSend_NoPeer_ReturnsFalse: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_TextSend_ReturnsTrue_WithActivePeer();
        std::cout << "[       OK ] TextSend_ReturnsTrue_WithActivePeer\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] TextSend_ReturnsTrue_WithActivePeer: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_SingleSlave_RejectsSecondClient();
        std::cout << "[       OK ] SingleSlave_RejectsSecondClient\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] SingleSlave_RejectsSecondClient: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_VideoStream_Delivery();
        std::cout << "[       OK ] VideoStream_Delivery\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] VideoStream_Delivery: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_AudioStream_Delivery();
        std::cout << "[       OK ] AudioStream_Delivery\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] AudioStream_Delivery: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_Robustness_PushBeforeConnect();
        std::cout << "[       OK ] Robustness_PushBeforeConnect\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] Robustness_PushBeforeConnect: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_Robustness_RapidReconnect();
        std::cout << "[       OK ] Robustness_RapidReconnect\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] Robustness_RapidReconnect: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_Robustness_StopWhilePushing();
        std::cout << "[       OK ] Robustness_StopWhilePushing\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] Robustness_StopWhilePushing: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_Robustness_InvalidAddress();
        std::cout << "[       OK ] Robustness_InvalidAddress\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] Robustness_InvalidAddress: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
    ...
    try
    {
        test_Robustness_InvalidCodecConfig();
        std::cout << "[       OK ] Robustness_InvalidCodecConfig\n"
                  << std::flush;
        num_tests_passed++;
    }
    catch (const std::exception &e)
    {
        std::cout << "[  FAILED  ] Robustness_InvalidCodecConfig: " << e.what() << "\n"
                  << std::flush;
        num_tests_failed++;
    }
