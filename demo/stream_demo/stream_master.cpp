...
public:
    ...
    bool open(const std::string& path)
    {
        ...
        if (ret < 0) { err_ = "avformat_open_input: " + avErr(ret); return false; }

        ret = avformat_find_stream_info(fmt_ctx_, nullptr);
        if (ret < 0) { err_ = "avformat_find_stream_info: " + avErr(ret); return false; }
        ...
        if (ret < 0) { err_ = "No video stream: " + avErr(ret); return false; }
        video_idx_ = ret;
        ...
        if (!codec) { err_ = "Decoder not found"; return false; }
        ...
        if (ret < 0) { err_ = "avcodec_open2: " + avErr(ret); return false; }
    ...
    double fps() const
    {
        if (!fmt_ctx_ || video_idx_ < 0) return 30.0;
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::string(argv[i]) == "--loop") { loop_mode = true; break; }
    }
